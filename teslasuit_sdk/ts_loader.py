...
class TsLoader:
    ...
    def load(self):
        if self.__path is not None and os.path.isfile(self.__path):
            if os.name == 'posix':
                self.__lib = C.CDLL(self.__path)
            else:
                self.__lib = C.WinDLL(self.__path)

        else:
            if os.name == 'posix':
                self.__lib = C.CDLL(find_library('teslasuit_api'))
            else:
                self.__lib = C.WinDLL(find_library('teslasuit_api'))
